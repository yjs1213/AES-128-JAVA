public class AES {
    private final int[][] state =new int[4][4]; // 4x4 행렬로, 메시지가 저장될 행렬입니다.
    private final int[][] keyary = new int[4][4]; // 마찬가지로 키가 저장될 행렬입니다.
    private final int[][] prekey = new int[11][16]; // 복호화시 키를 미리 계산하게 되는데 각 행마다 128비트의 키가 저장됩니다.
    ...
    public String decryptAES(String ciphert, String keys){
        ...
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                state[j][i]=Integer.parseInt(ciphert.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
                keyary[j][i]=Integer.parseInt(keys.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
            }
        }
